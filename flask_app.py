...
rooms = {}          # code → {players: [sids], names: {sid:name}, teams: {sid:team}, host: sid, settings: dict, max_players: int}
client_rooms = {}   # sid → code

def _room_info(code):
    r = rooms.get(code, {})
    return [{"sid": s, "name": r["names"].get(s, "?"), "team": r["teams"].get(s, 0)} for s in r.get("players", [])]
...
def _cleanup_client(sid):
    ...
    if r["host"] == sid and r["players"]:
        r["host"] = r["players"][0]
        emit("new_host", {"new_host": r["host"]}, room=code)

    # Notify room
    socketio.emit("player_left", {
        "sid": sid,
        "name": name,
        "players": _room_info(code),
        "new_host": r["host"]
    }, room=code)
...
def on_host_room(data):
    ...
    if not code:
        emit("host_error", {"reason": "invalid_code"})
        return

    if code in rooms:
        emit("host_error", {"reason": "code_taken"})
        return
    ...
    client_rooms[sid] = code
    join_room(code)
    join_room(sid)  # optional: join own SID room for future targeted emits
...
def on_join_room(data):
    ...
    client_rooms[sid] = code
    join_room(code)
    join_room(sid)  # optional: join own SID room
...
def on_relay(data):
    ...
    if target_sid:
        # Send to EVERYONE in the room — client filters by target_sid === mySid
        socketio.emit("relay", data, room=code)
        print(f"[RELAY] Targeted {target_sid} ← from {sid} (broadcast to room {code})")
    else:
        # Normal room broadcast
        socketio.emit("relay", data, room=code, skip_sid=sid)
        print(f"[RELAY] Room broadcast from {sid} in {code}")
